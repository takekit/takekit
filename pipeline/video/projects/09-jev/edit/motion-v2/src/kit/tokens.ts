 ...
 *
 * Os valores não são gosto. Saem de duas medições: `fusion_motion.py` (a curva
 * monotônica de entrada+deriva) e o motor de referência em
...
/**
 * Encaixe do palco B, beat a beat.
 *
 * A faixa útil do B tem 440 px (180..620): entre a zona morta da UI do Instagram
 * e a caption, que mora no vão de creme acima da cabeça do host. Cinco beats
 * nasceram mais altos que isso, então além de descer eles encolhem — e só o
 * quanto falta para caber.
 *
 * `shift` e `scale` saem de MEDIÇÃO do alfa renderizado por `tools/verify.py`,
 * que decodifica o clipe inteiro: `scale = 440 / altura_medida` e
 * `shift = 180 − topo_medido × scale`. Medir por amostra de frames não serve —
 * o extremo do percurso cai entre as amostras (foi assim que u01 e u07 passaram
 * no portão e ainda assim fecharam abaixo de 620). A escala tem origem no topo do eixo
 * central, então o gráfico encolhe para dentro sem sair do centro em x.
 *
 * Encolher NÃO é o jeito de fingir a descida (a descida é translação). É o
 * último recurso de quem não cabe: u04 é um disco sem tipo, e nos outros a perda
 * de corpo fica em ~13% no pior caso.
 */
export const STAGE_B_FIT: Record<string, {shift: number; scale: number}> = {
  u01: {shift: 104, scale: 0.94},
  u04: {shift: 135, scale: 0.793},
  u07: {shift: 37, scale: 0.95},
  u09: {shift: 17, scale: 1},
  u10: {shift: 55, scale: 1},
  u11: {shift: 82, scale: 0.878},
  u12: {shift: 94, scale: 1},
  u14: {shift: 105, scale: 0.864}, // +2 de folga: a deriva sobe ~1 px no fim
};
 ...
 *
 * Regra mestra (de `engine/fusion_motion.py`): entrada e deriva são UMA função
...
/** Duração exata de cada beat, em frames (src_out − src_in de cuts.json). */
export const BEAT_FRAMES: Record<string, number> = {
  u01: 57,
  u03: 40,
  u04: 32,
  u05: 75,
...
export const BEAT_STAGE: Record<string, 'B' | 'C'> = {
  u01: 'B',
  u03: 'C',
  u04: 'B',
  u05: 'C',
