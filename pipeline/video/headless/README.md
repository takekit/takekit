...
| Preview / export | render do Resolve | **`compose.py`** (FFmpeg): preview por default, final no botão Exportar |
...
| `soundEffects` | `compose.py`, `map_sfx_cues.py` | `sfx: false` desliga o sfx_map; `music` (arquivo relativo a `video/resolve/`, `gainDb`, `duck`; null = sem cama; `--music` ganha); `kindToCatalogId` |
...
Legendas: `captions_palco.py --job edit/job_<face|canvas|hold>.json --style video/resolve/styles/palco-<layer>.json
...
`video/resolve/engine/caption_preview.py --preset <json|-> --text "Isso muda *tudo*" --layout face|canvas
...
- **Enquadramento**: o rosto vai para o centro na horizontal, na altura mediana em que já está
  ...
  (0.5, 0.36 do topo = `[0.5, 0.64]` do `presets/punch.json` da Fusion).
...
[FUSION.md](../resolve/FUSION.md). O punch voltou como módulo `camera` (acima); shake, intro
zoom e os splits do build.json ainda precisam de port próprio antes de voltar a um vídeo.
