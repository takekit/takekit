...
import {H, PALETTE, STAGE_B, STAGE_B_FIT_DEFAULT, STAGE_C, STAGE_SHIFT, W} from './tokens';

export type BeatFit = {shift: number; scale: number};

/**
 * Qual beat está sendo desenhado e o encaixe medido. O Root informa; o palco
 * usa `fit` (de beats.json) ou o default. Fica aqui, e não numa prop, para
 * nenhum beat precisar carregar o próprio número de layout.
 */
export const BeatId = createContext<string>('');
export const BeatFitCtx = createContext<BeatFit>(STAGE_B_FIT_DEFAULT);
 ...
 * caption. Os números vêm de `beats.json` → `fit` (medidos por verify.py).
 */
...
const Encaixe: React.FC<{children: React.ReactNode}> = ({children}) => {
  const {shift, scale} = useContext(BeatFitCtx);
  return (
    <div
      style={{
        position: 'absolute',
