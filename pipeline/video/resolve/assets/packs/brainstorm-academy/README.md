...
| Recursos | Local relativo a `video/resolve/assets/` | Uso |
|---|---|---|
...
| 2 arquivos Premiere + 11 After Effects | `packs/brainstorm-academy/adobe/` | Referências originais Adobe; não convertidas para Fusion |
...
por cima, no Resolve/Fusion. A cor pode acompanhar o assunto, contraste e tratamento visual do
...
| `background-02-alpha.mov` | Grid transparente, ProRes com alpha | Sobre cor criada no Resolve/Fusion |
...
no Source Viewer e conferir em playback. Para estender, verificar a emenda antes de repetir.
Compor fundo, mídia e texto no Resolve/Fusion. Esses fundos são assets prontos do pack; usá-los
não significa gerar nossos próprios motions em um compositor paralelo.
...
selecionado no editor ou extraído como derivado do projeto.
...
Fontes: arquivos opcionais arquivados localmente. Escolher/registrar no Fusion só quando o design
pedir; não trocar as fontes ou o motor das captions aprovadas. Não foi encontrado termo individual
...
Os originais são `.prfpset`, `.mogrt` e `.ffx`. Não há `.comp` ou `.setting` Fusion neste pack.
Eles foram arquivados, não instalados nem validados como efeitos do Resolve.
...
| Pan contínuo e zoom contínuo | Transform + splines no Fusion, com tempo relativo à duração do clipe |
...
- Fundos inspecionados em frames extraídos; ainda sem avaliação de movimento no Resolve.
- Sem instalação de fontes, alterações de captions ou edição de timelines.
- Aprovação estética, audição dos novos SFX e teste de importação no Resolve pendentes de uso.
