...
HEADLESS = RESOLVE.parent / "headless"
sys.path.insert(0, str(HEADLESS))
sys.path.insert(0, str(RESOLVE / "engine"))
...
caption_jobs = _load("caption_jobs", RESOLVE / "pipeline" / "caption_jobs.py")
...
class CaptionPresetTests(unittest.TestCase):
    def setUp(self):
        self.face = cp.load_style(str(RESOLVE / "styles" / "palco-face.json"))
        self.hold = cp.load_style(str(RESOLVE / "styles" / "palco-hold.json"))
        self.preset = json.loads((RESOLVE.parents[2] / "styles" / "_presets" / "caption" / "contorno-bold.json")
                                 .read_text()) if (RESOLVE.parents[2] / "styles").is_dir() else None
