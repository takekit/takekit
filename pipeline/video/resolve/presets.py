...
"""Registro de presets — única fonte de verdade do que existe e do que foi aprovado.
  ...
  presets.py add <id> --kind K --file F [--status draft|approved] [--summary S] [--video V] [--menu M] [--engine E]
...
def cmd_add(a):
    ...
    d["presets"].append({"id": a.id, "kind": a.kind, "file": a.file, "engine": a.engine, "menu": a.menu, "status": a.status,
...
s.add_argument("--status", default="draft"); s.add_argument("--summary"); s.add_argument("--video"); s.add_argument("--menu"); s.add_argument("--engine")
