...
> Direção vigente em WORKFLOW.md. Números ligados ao footage 02/04 são calibrações
...
O contrato obrigatório de `WORKFLOW.md`, §3, define roteiro, montagem e revisão integral.
...
essa for a intenção do usuário. Verificar layout por frames do preview e movimento/áudio por playback.
...
|---|---|---|---|
| Vai aplicar qualquer coisa nova no export | Testa num **trecho curto** (`compose.py`, qualidade de preview) e captura frames reais com `headless/frame.py --sheet` antes de aplicar no vídeo inteiro | Render do vídeo todo custa ~1 min; um frame errado custa segundos. Vários bugs só apareceram no sheet (crop apagou o A-roll, gap de 1 frame, legenda invadindo split) | Abrir as capturas; assistir o export com som para motion |
...
| Acelerar 1.05x | Só se o usuário aprovar; fazer **na fonte** (ffmpeg atempo) e reescalar os cortes | Na fonte cada segmento continua uma unidade separada do cuts.json | frames = round(src/1.05); total bate |
| Split/dispositivo começa no meio de uma unidade | **Adiciona corte extra** naquele frame (nova unidade no cuts.json) | Palco é por unidade; o split não entra no meio dela | a unidade nova começa exatamente na palavra |
...
| situação | decisão | por quê | como verificar |
|---|---|---|---|
| **Enquadrar o apresentador na faixa de cima** | Janela do a-roll subida ~210 px, zoom 1.0 — rosto inteiro + queixo + parte do peito visíveis; nariz ~ 45 % da faixa | Cortar no queixo parece erro; zoom > 1 no split perde contexto de mão/gesto. Regra: **olhos no terço superior da faixa, queixo com folga** | still no meio do split: testa/olhos/queixo dentro; nada de "cabeça cortada" |
| **B-roll 16:9 na faixa de baixo** | Escala **1.58** (cover: 958 px de altura, corta laterais) | Faixa full-width sem barra preta (lei). Corte lateral de UI branca é invisível; barra preta é feia | frame: nenhum pixel preto na faixa; conteúdo central visível |
...
| situação | decisão | por quê | como verificar |
|---|---|---|---|
| Onde ancorar o zoom | Pivot no **rosto** (0.5, 0.36 do topo) | Zoom no centro geométrico puxa pro peito | frame no pico do zoom: rosto centralizado, não subiu |
...
Usar os critérios de revisão do WORKFLOW.md, §5: frames + playback, leitura, continuidade, voz e escopo
