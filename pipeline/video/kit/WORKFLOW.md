...
3. **Camada nova no projeto**: última opção. Gerar a camada (`.mov` com alfa, PIL/FFmpeg/Remotion)
   ...
   replicar. Acervos de terceiros ficam fora do processo.
...
estado existente sem sobrescrever dados nem migrar em massa. `compose.json` e jobs de captions
são artefatos de execução, não outro briefing. Reconciliar o
