...
> Tudo roda no pipeline headless ([../headless](../headless/README.md)): câmera pelo módulo
> `camera`, camadas em PIL/FFmpeg/Remotion, montagem pelo `compose.py` (`edit/compose.json` para exceções).

## Direção acordada com o Oldaque

Captions seguem o formato: explicação ilustrada usa as duas famílias + hold em
[FORMATOS.md](FORMATOS.md). Para outros recursos, estabelecer uma execução visual e sonora reconhecível:
...
- **Base visual existente:** módulo `camera` (`styles/_presets/camera/punch-na-enfase.json` e
  vizinhos). Referência aprovada no 02: zoom 1.14, variante suave 1.10, rampa de cerca de 7 frames
  a 30 fps, pivô na linha dos olhos. Os valores são ponto de partida; em outro fps preservar o
  tempo percebido.
- **Construção:** `video/headless/camera.py` — escala 1.0 → `intensity` em `punchFrames` no começo
  da unidade, segura até o corte; face tracking centra o rosto. Marca `"camera": "punch"` no beat
  força o movimento. No punch-out, verificar o enquadramento revelado.
...
O zoom-out de abertura é outro comportamento: na referência do 02 parte de 1.40 e chega a 1.00
com blur em 18 frames (`zoomOut` do módulo `camera`, sem o blur). Não confundir essa abertura mais
...
CTA, nem o uso de glass em outros vídeos.
...
3. Executar no pipeline headless: câmera pelo módulo `camera` (ou marca no beat), camadas em
   PIL/FFmpeg/Remotion, som em `edit/sfx_map.json` (`map_sfx_cues.py`); o SFX não é associado
   automaticamente ao movimento.
4. Conferir o preview (`compose.py`) em playback com som e frames com `frame.py`. Ajustar o
   tratamento conforme o contexto, preservando a família de caption do formato.
